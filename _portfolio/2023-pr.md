...
-- Filter 1: Minimum attempts per question (≥10). Questions must have been answered at least 10 times in order to obtain a reliable estimate of difficulty, 
filter1 AS (
  SELECT 
      course_id, 
      question_id,
...
-- Filter 2: Minimum unique users (≥10). Questions must have been answered by at least 10 different users in order to obtain a reliable estimate of difficulty.
filter2_counts AS (
  SELECT 
      course_id,
      question_id,
...
-- Filter 3: Minimum accuracy threshold (≥25% correct). Questions must be solvable at above chance levels in order to obtain a reliable estimate of difficulty.
filter3_counts AS (
  SELECT 
      course_id,
      question_id,
