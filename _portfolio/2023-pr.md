...
## Step 2. Create a summary table that holds student-, question-, and course-level columns needed for defining thresholds for filtering.

## Step 2a. Student-level summary (level_student) tracks each student's attempts and performance by question.
```sql
-- Student-level summary (level_student) tracks each student's attempts and performance by question.
level_student AS (
  SELECT 
      course_id,
      user_id,
...
## Step 2b. Question-level summary (level_question) aggregates attempts and accuracy across all students.
```sql
level_question AS (
  SELECT 
      course_id,
...
## Step 2c. Course-level summary counts questions and unique users per course.
```sql
level_course AS (
  SELECT 
      course_id,
...
## Step 3. Combine student, question, and course levels.
```sql
PR_level_sum AS (
  SELECT 
      s.*,
...
## Step 6a. Compute question difficulty.
```sql
qdiff AS (
  SELECT 
      course_id,
...
## Step 6b. Compute user mean question difficulty.
```sql
user_mean_qdiff AS (
  SELECT 
      course_id,
