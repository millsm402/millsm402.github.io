...
Adaptive testing is an assessment approach where the difficulty of questions dynamically adjusts in real time based on the test-taker’s responses. Within Problem Roulette, adaptive testing enhances the student experience by allowing learners to optimize their study sessions, focusing on the right number and difficulty of questions to meet their personal learning goals rather than working through a fixed set.

The foundation of building this adaptive testing feature begins with accurately measuring the difficulty of each question. By quantifying difficulty, we provide the question selection algorithm with the critical data it needs to make informed decisions about which question to serve next. This step ensures that students are consistently challenged at an appropriate level—neither overwhelmed by overly difficult problems nor under-stimulated by questions they’ve already mastered.

Beyond measuring difficulty, the system must also track individual performance trends, adapt question pacing, and incorporate a feedback loop to refine both student proficiency estimates and question difficulty ratings over time. Ultimately, these elements combine to create a personalized, data-driven learning environment that helps students study more efficiently and effectively.

My pipeline below builds the foundation for adaptive testing in Problem Roulette by systematically computing question-level difficulty scores and user-level proficiency estimates. It first consolidates raw question and session data, aggregates performance at multiple levels (student, question, and course), and applies strict quality filters to ensure reliability. The final outputs—difficulty scores for every question and average difficulty estimates for each user—feed directly into the adaptive testing engine. These metrics enable the system to dynamically match students with questions at the appropriate challenge level, creating a feedback loop where each subsequent question is tailored to the learner’s current ability, maximizing study efficiency and engagement.

This new system powers Problem Roulette’s adaptive practice mode, where each question you see is personalized to your skill level. Behind the scenes, our data pipeline analyzes thousands of student responses, measuring how challenging each question is and estimating your average comfort level. These scores help the system choose questions that keep you in your “learning zone”—not too easy, not too hard—so every session is tailored to maximize your efficiency and boost your confidence.
...
## Step 2. Create summary tables that hold student-, question-, and course-level columns needed for defining thresholds for filtering.
...
## Step 3. Combine student, question, and course levels. Then setup filters. Then merge.
```sql
PR_level_sum AS (
  SELECT 
      s.*,
