...
**Key Takeaways** 
(1) **More questions helps**, so long as you’re getting them correct (i.e., more positive slope for high accuracy (green) than medium (red), which in turn is more positive than low (blue)). **More questions hurts** GPE when answering incorrectly. This is especially true the more difficult the questions are.

(2) **More accurate answers almost always helps**. Accuracy matters less the fewer number of questions completed.

(3) **Question difficulty is not super important on its own** (at least not after controlling for the other dimensions), which can be seen by the similar heights of each colored line. If anything, this plot (nbin=3) suggests that harder questions are a bit more helpful (each colored line is a bit higher than the corresponding one in the third (Bin_UserMeanItemDifficulty=3…“easy”), unless accuracy is poor (in which case difficulty does not really matter).
