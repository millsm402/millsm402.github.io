...
(3) **Question difficulty is not super important on its own**, which can be seen by the similar heights of each colored line. If anything, this plot suggests that harder questions are a bit more helpful (each colored line is a bit higher than the corresponding one as questions get easier, unless accuracy is low in which case difficulty does not really matter).
