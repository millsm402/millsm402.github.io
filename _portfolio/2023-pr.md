...
(1) **More questions helps**, so long as you’re getting them correct (i.e., more positive slope for high accuracy (green) than medium (red) or low (blue)). **More questions hurts** GPE when answering incorrectly.
...
These key takeaways were then used to identify thresholds for sending personalized study tips. For example, if a user was answering a high number of easy questions with high accuracy, they would receive a tip to study harder questions. If instead a user was answering a high number of hard questions with low accuracy, they would receive a tip to study more intentionally. If a user were answering a medium number of medium difficulty questions with medium accuracy, they would receive a tip to study more questions of higher difficulty with more intentionality. Ultimately, users received tips on the dimension(s) that they could improve upon.
