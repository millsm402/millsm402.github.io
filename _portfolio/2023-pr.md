...
```sql
-- Core pipeline steps (simplified view):
-- 1. Build dataset (userQS) from raw user/question/session data.
-- 2. Summarize by student, question, and course.
-- 3. Apply quality filters (attempts, users, accuracy).
-- 4. Compute difficulty scores for each question and proficiency for each user.
