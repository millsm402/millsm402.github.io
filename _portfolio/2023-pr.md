...
(3) **Question difficulty is not super important on its own** (at least not after controlling for the other dimensions), which can be seen by the similar heights of each colored line. If anything, this plot suggests that harder questions are a bit more helpful (each colored line is a bit lower than the corresponding one as questions become more difficulty, unless accuracy is low in which case difficulty does not really matter).
