...
The first step in building an adaptive testing algorithm was to measure question difficulty for each question
...
graph TD
  A[user_question + user_session] --> B[userQS]
  B --> C[level_student]
  B --> D[level_course]
  C --> E[level_question]
  C --> F[PR_level_sum]
  E --> F
  D --> F
  F --> G[Filters (Attempts, Users, Accuracy)]
  G --> H[qdiff (Question Difficulty)]
  H --> I[user_mean_qdiff (User Mean Difficulty)]
<three backticks>
